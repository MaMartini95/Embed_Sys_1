...
bool CServer::transmitStateVectorData(SStateVectorData& data)
{
	SContent content;
	content.mStateData = data;
	return this->transmitMessage(content);
}
void CServer::init()
{
...
bool CServer::isConnectionValid()
{
	char buffer;
	return (recv(mConnectedSocketFD, &buffer, sizeof(buffer), MSG_PEEK | MSG_DONTWAIT) != 0);
}
